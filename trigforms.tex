...
\item [{Period:}] $2\pi$
\item [{VA:}] none
\item [{$\boldsymbol{x}$-intercepts:}] $k\pi$ where $k\in\mathbb{Z}$
\item [{Parity:}] odd
\end{description}
...
\item [{Period:}] $2\pi$
\item [{VA:}] none
\item [{$\boldsymbol{x}$-intercepts:}] $\frac{\pi}{2}+k\pi$ where $k\in\mathbb{Z}$
\item [{Parity:}] even
\end{description}
...
\item [{$\boldsymbol{x}$-intercepts:}] none
\item [{Parity:}] odd
\end{description}
...
where $c=\pm\sqrt{a^{2}+b^{2}+2ab\cos\theta}$, and $\varphi$ satisfies
...
\pgfkeys{/pgfplots/Axis Style/.style={ width=10cm, height=6cm, axis x line=center, axis y line=middle, samples=100, ymin=-2.5, ymax=2.5, xmin=-7.0, xmax=7.0, domain=-2*pi:2*pi, ytick={-2,-1,1,2} }}


\subsection{$y=\sin x$}

\begin{tikzpicture} \begin{axis}[ Axis Style, xtick={-6.28318, -4.7123889, -3.14159,-1.5708, 1.5708, 3.14159, 4.7123889, 6.28318 }, xticklabels={ $-2\pi$, $-\frac{3\pi}{2}$, $-\pi$, $-\frac{\pi}{2}$, $\frac{\pi}{2}$, $\pi$, $\frac{3\pi}{2}$, $2\pi$ } ] \addplot [mark=none, thick, blue] {sin(deg(x))}; \end{axis} \end{tikzpicture}


\subsection{$y=\cos x$}

\begin{tikzpicture} \begin{axis}[ Axis Style, xtick={ -6.28318, -4.7123889, -3.14159, -1.5708, 1.5708, 3.14159, 4.7123889, 6.28318 }, xticklabels={ $-2\pi$, $-\frac{3\pi}{2}$, $-\pi$, $-\frac{\pi}{2}$, $\frac{\pi}{2}$, $\pi$, $\frac{3\pi}{2}$, $2\pi$ } ] \addplot [mark=none, thick, blue] {cos(deg(x))}; \end{axis} \end{tikzpicture}


\subsection{$y=\tan x$}

\begin{tikzpicture} \begin{axis}[ Axis Style, xtick={ -6.28318, -4.7123889, -3.14159, -1.5708, 1.5708, 3.14159, 4.7123889, 6.28318 }, xticklabels={ $-2\pi$, $-\frac{3\pi}{2}$, $-\pi$, $-\frac{\pi}{2}$, $\frac{\pi}{2}$, $\pi$, $\frac{3\pi}{2}$, $2\pi$ } ] \addplot [domain=-2*pi:-3*pi/2-0.1,mark=none, thick, blue] {tan(deg(x))}; \addplot [domain=-3*pi/2+0.1:-pi/2-0.1,mark=none, thick, blue] {tan(deg(x))}; \addplot [domain=-pi/2+0.1:pi/2-0.1,mark=none, thick, blue] {tan(deg(x))}; \addplot [domain=pi/2+0.1:3*pi/2-0.1,mark=none, thick, blue] {tan(deg(x))}; \addplot [domain=3*pi/2+0.1:2*pi,mark=none, thick, blue] {tan(deg(x))}; \end{axis} \end{tikzpicture}


\subsection{$y=\csc x$}

\begin{tikzpicture} \begin{axis}[ Axis Style, xtick={ -6.28318, -4.7123889, -3.14159, -1.5708, 1.5708, 3.14159, 4.7123889, 6.28318 }, xticklabels={ $-2\pi$, $-\frac{3\pi}{2}$, $-\pi$, $-\frac{\pi}{2}$, $\frac{\pi}{2}$, $\pi$, $\frac{3\pi}{2}$, $2\pi$ } ] \addplot [domain=-2*pi+0.1:-pi-0.1,mark=none, thick, blue] {cosec(deg(x))}; \addplot [domain=-pi+0.1:-0.1,mark=none, thick, blue] {cosec(deg(x))}; \addplot [domain=0.1:pi-0.1,mark=none, thick, blue] {cosec(deg(x))}; \addplot [domain=pi+0.1:2*pi-0.1,mark=none, thick, blue] {cosec(deg(x))}; \end{axis} \end{tikzpicture}


\subsection{$y=\sec x$}

\begin{tikzpicture} \begin{axis}[ Axis Style, xtick={ -6.28318, -4.7123889, -3.14159, -1.5708, 1.5708, 3.14159, 4.7123889, 6.28318 }, xticklabels={ $-2\pi$, $-\frac{3\pi}{2}$, $-\pi$, $-\frac{\pi}{2}$, $\frac{\pi}{2}$, $\pi$, $\frac{3\pi}{2}$, $2\pi$ } ] \addplot [domain=-2*pi:-3*pi/2-0.1,mark=none, thick, blue] {sec(deg(x))}; \addplot [domain=-3*pi/2+0.1:-pi/2-0.1,mark=none, thick, blue] {sec(deg(x))}; \addplot [domain=-pi/2+0.1:pi/2-0.1,mark=none, thick, blue] {sec(deg(x))}; \addplot [domain=pi/2+0.1:3*pi/2-0.1,mark=none, thick, blue] {sec(deg(x))}; \addplot [domain=3*pi/2+0.1:2*pi,mark=none, thick, blue] {sec(deg(x))}; \end{axis} \end{tikzpicture}


\subsection{$y=\cot x$}

\begin{tikzpicture} \begin{axis}[ Axis Style, xtick={ -6.28318, -4.7123889, -3.14159, -1.5708, 1.5708, 3.14159, 4.7123889, 6.28318 }, xticklabels={ $-2\pi$, $-\frac{3\pi}{2}$, $-\pi$, $-\frac{\pi}{2}$, $\frac{\pi}{2}$, $\pi$, $\frac{3\pi}{2}$, $2\pi$ } ] \addplot [domain=-2*pi+0.1:-pi-0.1,mark=none, thick, blue] {cot(deg(x))}; \addplot [domain=-pi+0.1:-0.1,mark=none, thick, blue] {cot(deg(x))}; \addplot [domain=0.1:pi-0.1,mark=none, thick, blue] {cot(deg(x))}; \addplot [domain=pi+0.1:2*pi-0.1,mark=none, thick, blue] {cot(deg(x))}; \end{axis} \end{tikzpicture}
...
Let $s$ be the semiperimeter, that is, $s=\frac{a+b+c}{2}$, and
...
\noindent \begin{center}
${\displaystyle \frac{a+b}{c}=\frac{\cos\frac{\alpha-\beta}{2}}{\sin\frac{\gamma}{2}}}$;$\quad{\displaystyle \frac{a-b}{c}=\frac{\sin\frac{\alpha-\beta}{2}}{\cos\frac{\gamma}{2}}}$
\par\end{center}
...
Solving right triangles is simply using the definitions of the trigonometric
functions and the Pythagorean theorem to determine the other parts.
The right angle $\gamma=90^{\circ}$ is always assumed to be given.
...
\begin{cases}
r=\sqrt{x^{2}+y^{2}}\\
\theta=\tan^{-1}\left(y,x\right)\approx\tan^{-1}\left(\frac{y}{x}\right)
\end{cases}
\]
...
\subsection{Limaçon of Pascal}

A polar equation of the form $r=a+b\cos\theta$ or $r=a+b\sin\theta$
has a polar graph which is called a \emph{limaçon}.
...
\subsubsection*{Types of Limaçons}
\begin{itemize}
\item Looped limaçon if $0<\left|\frac{a}{b}\right|<1$\\
...
\item Dimpled limaçon if $1<\left|\frac{a}{b}\right|<2$\\
...
\item Convex limaçon if $\left|\frac{a}{b}\right|\ge2$\\
...
\subsubsection*{Symmetry and Direction}
...
\begin{tabular}{|c|c|c|}
\hline 
 & symmetry & points to the\tabularnewline
\hline 
\hline 
$r=a+b\cos\theta$ & polar axis & right\tabularnewline
\hline 
$r=a-b\cos\theta$ & polar axis & left\tabularnewline
\hline 
$r=a+b\sin\theta$ & $\frac{\pi}{2}$-axis & top\tabularnewline
\hline 
$r=a-b\sin\theta$ & $\frac{\pi}{2}$-axis & bottom\tabularnewline
\hline 
\end{tabular}
\par\end{center}
...
\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar]
\addplot[color=blue,domain=0:360, samples=100]{5*cos(3*x)};
...
\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar]
\addplot[color=blue,domain=0:360, samples=200]{2*cos(4*x)};
\end{polaraxis} \end{tikzpicture}
\par\end{center}
...
The polar graph of a polar equation $r=a\theta$ where $\theta>0$
and $a\in\mathbb{R}$ is called a \emph{spiral}.

\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar]
\addplot[color=blue,domain=0:720, samples=100]{x/120};
\end{polaraxis} \end{tikzpicture}
\par\end{center}
...
\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar] 
\addplot[color=blue,domain=-180:-135,samples=100]{4*sqrt(cos(2*x))}; \addplot[color=blue,domain=-45:45,samples=100]{4*sqrt(cos(2*x))}; \addplot[color=blue,domain=135:180,samples=100]{4*sqrt(cos(2*x))};
\end{polaraxis} \end{tikzpicture}
\par\end{center}
...
\subsection{Folium of Descartes}

A \emph{folium} is a plane curve proposed by Descartes to challenge
Fermat's extremum-finding techniques. It has a polar equation $r=\frac{3a\sec\theta\tan\theta}{1+\tan^{3}\theta}$.

\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar] \addplot[color=blue,domain=-35:89.9, samples=200]{(3*tan(x)*sec(x)/(1+(tan(x))^3)};  \addplot[color=blue,domain=90.1:125, samples=200]{(3*tan(x)*sec(x)/(1+(tan(x))^3)};\end{polaraxis} \end{tikzpicture}
\par\end{center}


\subsection{Spiral of Fermat}

The \emph{Fermat's spiral}, also known as the parabolic spiral, has
a polar equation $r^{2}=a^{2}\theta$. The resulting spiral is symmetric
with respect to the origin. 

\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar] \addplot[color=blue,domain=0:720, samples=100]{sqrt(x/120)}; \addplot[color=blue,domain=0:720, samples=100]{-sqrt(x/120)}; \end{polaraxis} \end{tikzpicture}
\par\end{center}


\subsection{Cissoid of Diocles}

Given an origin $O$ and a point $P$ on the curve, let $S$ be the
point where the extension of the line $OP$ intersects the line $x=2a$
and $R$ be the intersection of the circle of radius $a$ and center
$\left(a,0\right)$ with the extension of $OP$. Then the \emph{cissoid
of Diocles} is the curve which satisfies $OP=RS$. It has a polar
equation $r=2a\sin\theta\tan\theta$.

\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar]  \addplot[color=blue,domain=110:170, samples=200]{2*sin(x)*tan(x)}; \addplot[color=blue,domain=190:250, samples=200]{2*sin(x)*tan(x)};  \end{polaraxis} \end{tikzpicture}
\par\end{center}


\subsection{Epispiral}

The \emph{epispiral} is a plane curve with a polar equation $r=a\sec\left(n\theta\right)$.
Then there are $n$ sections if $n$ is odd (in blue), or $2n$ sections
if $n$ is even (in red). A slightly more symmetric version considers
instead $r=a\left|\sec\left(n\theta\right)\right|$.

\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar] \addplot[color=blue,domain=    0: 29-3, samples=100]{3/2*sec(3*x)}; \addplot[color=blue,domain=	31+3: 89-3, samples=100]{3/2*sec(3*x)}; \addplot[color=blue,domain= 91+3:149-3, samples=100]{3/2*sec(3*x)}; \addplot[color=blue,domain=151+3:209-3, samples=100]{3/2*sec(3*x)}; \addplot[color=blue,domain=211+3:269-3, samples=100]{3/2*sec(3*x)}; \addplot[color=blue,domain=271+3:329-3, samples=100]{3/2*sec(3*x)}; \addplot[color=red,dash pattern=on 3pt off 2pt,domain=    0: 45-4, samples=100]{sec(2*x)}; \addplot[color=red,dash pattern=on 3pt off 2pt,domain= 45+4:135-4, samples=100]{sec(2*x)}; \addplot[color=red,dash pattern=on 3pt off 2pt,domain=135+4:225-4, samples=100]{sec(2*x)}; \addplot[color=red,dash pattern=on 3pt off 2pt,domain=225+4:315-4, samples=100]{sec(2*x)}; \addplot[color=red,dash pattern=on 3pt off 2pt,domain=315+4:360  , samples=100]{sec(2*x)}; \end{polaraxis} \end{tikzpicture}
\par\end{center}
...
The \emph{lituus} is the locus of the point $P$ moving such that
the area of a circular sector remains constant. It means a ``crook,''
in the sense of a bishop's crosier. It has a polar equation $r^{2}\theta=a^{2}$.

\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar]
\addplot[color=blue,domain=0:1800, samples=300]{sqrt(25/x)};
\end{polaraxis} \end{tikzpicture}
\par\end{center}


\subsection{Eight Curve}

The eight curve, also known as the \emph{lemniscate of Gerono, }is
given by the polar equation $r^{2}=a^{2}\sec^{4}\theta\cos\left(2\theta\right)$.
It has vertical tangents $\left(\pm a,0\right)$ and horizontal tangents
at $\left(\pm\frac{\sqrt{2}}{2}a,\pm\frac{1}{2}a\right)$.

\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar] 
\addplot[color=blue,domain=-180:-135,samples=100]{sqrt(16*(sec(x)^4)*cos(2*x))};
\addplot[color=blue,domain=-45:45,samples=100]{sqrt(16*(sec(x)^4)*cos(2*x))};
\addplot[color=blue,domain=135:180,samples=100]{sqrt(16*(sec(x)^4)*cos(2*x))};
\end{polaraxis} \end{tikzpicture}
\par\end{center}
...
\subsection{Butterfly Curve}

The \emph{butterfly curve} is a transcendental plane curve discovered
by Temple H. Fay. The curve is given by the polar equation $r=e^{\sin\theta}-2\cos4\theta+\sin^{5}\left(\frac{1}{24}\left(2\theta-\pi\right)\right)$.

\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar] \addplot[color=blue,domain=0:360, samples=400]{exp(sin(x))-2*cos(4*x)+(sin((2*x-pi)/24))^5}; \end{polaraxis} \end{tikzpicture}
\par\end{center}


\subsection{Strophoid}

Let $C$ be a curve, let $O$ be a fixed point (the pole), and let
$O'$ be a second fixed point. Let $P$ and $P'$ be points on a line
through $O$ meeting $C$ at $Q$ such that $P'Q=QP=QO'$. The locus
of $P$ and $P'$ is called the \emph{strophoid} of $C$ with respect
to the pole $O$ and fixed point $O'$. Its polar equation is $r=\frac{b\sin\left(a-2\theta\right)}{\sin\left(a-\theta\right)}$.

\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar] \addplot[color=blue,domain=0:24, samples=400]{(4*sin(30-2*x))/(sin(30-x))}; \addplot[color=blue,domain=50:202, samples=400]{(4*sin(30-2*x))/(sin(30-x))};
\end{polaraxis} \end{tikzpicture}
\par\end{center}
...
\subsection{Cochleoid}

A \emph{cochleoid} is a snail-shaped curve similar to a strophoid
which can be represented by the polar equation $r=\frac{a\sin\theta}{\theta}$.

\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar] \addplot[color=blue,domain=0.1:720, samples=500]{(200*sin(x))/x};
\end{polaraxis} \end{tikzpicture}
\par\end{center}


\subsection{Cycloid of Ceva}

The \emph{cycloid of Ceva} is a polar curve that can be used for angle
trisection. It has the polar equation $r=1+2\cos2\theta$.

\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar]
\addplot[color=blue,domain=0:360, samples=100]{1+2*cos(2*x)};
\end{polaraxis} \end{tikzpicture}
\par\end{center}
...
\subsection{Freeth's Nephroid}

It is a strophoid of a circle with the pole $O$ at the center of
the circle and the fixed point $P$ on the circumference of the circle.
It has a polar equation $r=a\left(1+2\sin\frac{\theta}{2}\right)$.

\noindent \begin{center}
\begin{tikzpicture} \begin{polaraxis}[my style polar] \addplot[color=blue,domain=0:720, samples=400]{1*(1+2*sin(x/2))}; \end{polaraxis} \end{tikzpicture}
\par\end{center}
...
A Pythagorean triple $\left(a,b,c\right)$ is a triple of positive
integers such that $a^{2}+b^{2}=c^{2}$ . It is primitive if the greatest
common divisor of $a$, $b$, and $c$ is 1. For any primitive Pythagorean
triple, either $m$ or $n$ is even, but not both (i.e. $m\not\equiv n\bmod2$).
When that is done, then every primitive Pythagorean triple $\left(a,b,c\right)$
is of the form $\left(a,b,c\right)=\left(m^{2}-n^{2},2mn,m^{2}+n^{2}\right)$
where $m$ and $n$ are relatively prime (i.e. $\gcd\left(m,n\right)=1$)
and $1\le n<m$.
...
$m$ & $n$ & $a$ & $b$ & $c$\tabularnewline
\hline 
2 & 1 & 3 & 4 & 5\tabularnewline
\hline 
3 & 2 & 5 & 12 & 13\tabularnewline
\hline 
4 & 1 & 15 & 8 & 17\tabularnewline
4 & 3 & 7 & 24 & 25\tabularnewline
\hline 
5 & 2 & 21 & 20 & 29\tabularnewline
5 & 4 & 9 & 40 & 41\tabularnewline
\hline 
6 & 1 & 35 & 12 & 37\tabularnewline
6 & 3 & 27 & 36 & 45\tabularnewline
6 & 5 & 11 & 60 & 61\tabularnewline
\hline 
7 & 2 & 45 & 28 & 53\tabularnewline
7 & 4 & 33 & 56 & 65\tabularnewline
7 & 6 & 13 & 84 & 85\tabularnewline
\hline 
8 & 1 & 63 & 16 & 65\tabularnewline
8 & 3 & 55 & 48 & 73\tabularnewline
8 & 5 & 39 & 80 & 89\tabularnewline
8 & 7 & 15 & 112 & 113\tabularnewline
\hline 
9 & 2 & 77 & 36 & 85\tabularnewline
9 & 4 & 65 & 72 & 97\tabularnewline
9 & 6 & 45 & 108 & 117\tabularnewline
9 & 8 & 17 & 144 & 145\tabularnewline
\hline 
10 & 1 & 99 & 20 & 101\tabularnewline
10 & 3 & 91 & 60 & 109\tabularnewline
10 & 5 & 75 & 100 & 125\tabularnewline
10 & 7 & 51 & 140 & 149\tabularnewline
10 & 9 & 19 & 180 & 181\tabularnewline
\hline 
11 & 2 & 117 & 44 & 125\tabularnewline
11 & 4 & 105 & 88 & 137\tabularnewline
11 & 6 & 85 & 132 & 157\tabularnewline
\end{tabular}$\quad$%
\begin{tabular}{cc|ccc}
$m$ & $n$ & $a$ & $b$ & $c$\tabularnewline
\hline 
11 & 8 & 57 & 176 & 185\tabularnewline
11 & 10 & 21 & 220 & 221\tabularnewline
\hline 
12 & 1 & 143 & 24 & 145\tabularnewline
12 & 3 & 135 & 72 & 153\tabularnewline
12 & 5 & 119 & 120 & 169\tabularnewline
12 & 7 & 95 & 168 & 193\tabularnewline
12 & 9 & 63 & 216 & 225\tabularnewline
12 & 11 & 23 & 264 & 265\tabularnewline
\hline 
13 & 2 & 165 & 52 & 173\tabularnewline
13 & 4 & 153 & 104 & 185\tabularnewline
13 & 6 & 133 & 156 & 205\tabularnewline
13 & 8 & 105 & 208 & 233\tabularnewline
13 & 10 & 69 & 260 & 269\tabularnewline
13 & 12 & 25 & 312 & 313\tabularnewline
\hline 
14 & 1 & 195 & 28 & 197\tabularnewline
14 & 3 & 187 & 84 & 205\tabularnewline
14 & 5 & 171 & 140 & 221\tabularnewline
14 & 7 & 147 & 196 & 245\tabularnewline
14 & 9 & 115 & 252 & 277\tabularnewline
14 & 11 & 75 & 308 & 317\tabularnewline
14 & 13 & 27 & 364 & 365\tabularnewline
\hline 
15 & 2 & 221 & 60 & 229\tabularnewline
15 & 4 & 209 & 120 & 241\tabularnewline
15 & 6 & 189 & 180 & 261\tabularnewline
15 & 8 & 161 & 240 & 289\tabularnewline
15 & 10 & 125 & 300 & 325\tabularnewline
15 & 12 & 81 & 360 & 369\tabularnewline
15 & 14 & 29 & 420 & 421\tabularnewline
\end{tabular}
...
\subsubsection*{Barycenter/Centroid}

The geometric centroid (center of mass) of the polygon vertices of
a triangle is the point $G$ which is also the intersection of the
triangle's three triangle medians. The point is therefore sometimes
called the median point. The centroid is always in the interior of
the triangle.

For a triangle with Cartesian vertices $\left(x_{1},y_{1}\right)$,
$\left(x_{2},y_{2}\right)$, $\left(x_{3},y_{3}\right)$, the Cartesian
coordinates of the centroid are given by:

\noindent \begin{center}
${\displaystyle G\left(x_{0},y_{0}\right)=\left(\frac{x_{1}+x_{2}+x_{3}}{3},\frac{y_{1}+y_{2}+y_{3}}{3}\right)}$
\par\end{center}


\subsubsection*{Incenter}

The incenter $I$ is the center of the incircle for a triangle. The
corresponding radius of the incircle is known as the inradius.

The incenter can be constructed as the intersection of angle bisectors.
It is also the interior point for which distances to the sides of
...
\noindent \begin{center}
${\displaystyle I\left(x_{0},y_{0}\right)=\left(\frac{ax_{1}+bx_{2}+cx_{3}}{a+b+c},\frac{ay_{1}+by_{2}+cy_{3}}{a+b+c}\right)}$
\par\end{center}


\subsubsection*{Orthocenter}

The intersection $H$ of the three altitudes $AH_{A}$, $BH_{B}$,
and $CH_{C}$ of a triangle is called the orthocenter.

For a triangle with Cartesian vertices $\left(x_{1},y_{1}\right)$,
$\left(x_{2},y_{2}\right)$, $\left(x_{3},y_{3}\right)$, the Cartesian
coordinates of the circumcenter are given by:

\noindent \begin{center}
${\displaystyle H\left(x_{0},y_{0}\right)=\left(\frac{x_{1}t_{A}+x_{2}t_{B}+x_{3}t_{C}}{t_{A}+t_{B}+t_{C}},\frac{y_{1}t_{A}+y_{2}t_{B}+y_{3}t_{C}}{t_{A}+t_{B}+t_{C}}\right)}$
\par\end{center}

where $t_{A},t_{B},t_{C}$ is equal to $\tan A,\tan B,\tan C$ respectively.
...
\noindent \begin{center}
${\displaystyle O\left(x_{0},y_{0}\right)=\left(O_{x},O_{y}\right)}$
\par\end{center}

where $O_{x}=\frac{(y_{2}-y_{3})\left(x_{1}^{2}+(y_{1}-y_{2})(y_{1}-y_{3})\right)+x_{2}^{2}(y_{3}-y_{1})+x_{3}^{2}(y_{1}-y_{2})}{2(x_{1}(y_{2}-y_{3})+x_{2}(y_{3}-y_{1})+x_{3}(y_{1}-y_{2}))}$
and $O_{y}=\frac{2(x_{2}-x_{3})\left(-x_{1}^{2}+x_{3}^{2}-y_{1}^{2}+y_{3}^{2}\right)+2(x_{1}-x_{3})\left(x_{2}^{2}-x_{3}^{2}+y_{2}^{2}-y_{3}^{2}\right)}{4(x_{1}(y_{2}-y_{3})+x_{2}(y_{3}-y_{1})+x_{3}(y_{1}-y_{2}))}$.


\subsubsection*{Excenter}

An excenter, denoted $J_{i}$, is the center of an excircle of a triangle.
An excircle is a circle tangent to the extensions of two sides and
the third side. It is also known as an escribed circle. 

\newpage{}


\subsection{Area of the Triangle}


\subsubsection*{Base and Altitude}
...
\subsubsection*{SAS Triangle}

\[
A_{\triangle}=\frac{1}{2}ab\sin\gamma=\frac{1}{2}bc\sin\alpha=\frac{1}{2}ac\sin\beta
\]



\subsubsection*{SSS Triangle}

Given three sides $a$, $b$, and $c$, the area of the triangle can
be determined by Heron's formula:

\begin{eqnarray*}
A_{\triangle} & = & \sqrt{s\left(s-a\right)\left(s-b\right)\left(s-c\right)}\\
 & = & \frac{1}{4}\sqrt{4a^{2}b^{2}-(a^{2}+b^{2}-c^{2})^{2}}
\end{eqnarray*}


where $s=\frac{a+b+c}{2}$ is the semiperimeter. The area can also
be determined by:
...
\subsubsection*{AAS Triangle}

\[
A_{\triangle}=\frac{a^{2}\sin\beta\sin\gamma}{2\sin\alpha}=\frac{b^{2}\sin\alpha\sin\gamma}{2\sin\beta}=\frac{c^{2}\sin\alpha\sin\beta}{2\sin\gamma}
\]


where the missing angle can be easily determined through $\alpha+\beta+\gamma=180^{\circ}$.


\subsubsection*{ASA Triangle}

\[
A_{\triangle}=\frac{a^{2}}{2\left(\cot\beta+\cot\gamma\right)}=\frac{b^{2}}{2\left(\cot\alpha+\cot\gamma\right)}=\frac{c^{2}}{2\left(\cot\alpha+\cot\beta\right)}
\]



\subsubsection*{Three Vertices}

Given the vertices of the triangle $\left(x_{i},y_{i}\right)\;\forall i\in\left\{ 1,2,3\right\} $,
the shoelace formula can be applied for $n=3$ as follows:

\begin{eqnarray*}
A_{\triangle} & = & \frac{1}{2}\left|\sum_{i=1}^{2}x_{i}y_{i+1}+x_{n}y_{1}-\sum_{i=1}^{2}x_{i+1}y_{i}-x_{1}y_{n}\right|\\
 & = & \frac{1}{2}\left|x_{1}y_{2}+x_{2}y_{3}+x_{3}y_{1}-x_{2}y_{1}-x_{3}y_{2}-x_{1}y_{3}\right|
\end{eqnarray*}


However, if one of the vertices is at the origin, then the area of
the triangle can be simplified:

\[
A_{\triangle}=\frac{1}{2}\left\Vert \begin{array}{cc}
x_{2} & x_{3}\\
y_{2} & y_{3}
\end{array}\right\Vert =\frac{1}{2}\left|x_{2}y_{3}-x_{3}y_{2}\right|
\]


where $\left\Vert \cdot\right\Vert $ denotes the absolute value of
the determinant.
...
\[
A_{\triangle}=\frac{b}{4}\sqrt{4a^{2}-b^{2}}=\frac{1}{2}a^{2}\sin\theta
\]
...
\[
A_{\triangle}=I+\frac{1}{2}B-1
\]


where $I$ is the number of internal lattice points and $B$ is the
number of lattice points lying on the border of the triangle.
